// ══════════════════════════════════════════════════════════════
//  NV Field Guide — Google Apps Script Backend
//  Paste this entire file into your Apps Script editor,
//  then deploy as a Web App (see setup guide).
// ══════════════════════════════════════════════════════════════

const SHEET_ID   = '1rty4an6ti6vxOzcsfwhm146REhckTlnNPt723wLoRkM'; // ← replace after creating the sheet
const SHEET_NAME_ENTRIES  = 'Entries';
const SHEET_NAME_COMMENTS = 'Comments';
const SHEET_NAME_LIKES    = 'Likes';  // ← NEW: spot_name | likes
  ...
  try {
    ...
    if (action === 'place_info') {
      ...
      try {
        ...
        // Description meta tag — contains rating, price, type, hours
        ...
        if (metaDesc) {
          ...
          var cMatch = desc.match(/·\s*([A-Za-z\s]{3,40}(?:restaurant|bar|café|cafe|cantina|pub|lounge|club|beach|park|garden|market|gallery|museum|shop|store|salon))/i);
          if (cMatch) info.place_type = cMatch[1].trim();
        }

        // JSON-LD structured data (richer, when available)
...
function doPost(e) {
  try {
    ...
    // ── NEW: Like / unlike a spot ──────────────────────────
    if (action === 'like') {
      const sheet   = ensureLikesSheet(ss);
      const spotName = body.spot_name || '';
      const delta   = parseInt(body.delta) || 1; // +1 or -1
      ...
      // Not found — create row (only if it's a like, not unlike)
      if (!found && delta > 0) {
